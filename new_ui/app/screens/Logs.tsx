...
import { Info, AlertTriangle, XCircle, Search, Filter } from "lucide-react";
              ...
              {filteredLogs.length === 0 ? (
                ...
                filteredLogs.map(log => (
                  <tr key={log.id} className="hover:bg-[#222]/50 transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-neutral-300 truncate max-w-[200px]" title={log.asset}>{log.asset}</td>
                    <td className="px-4 py-2.5 text-neutral-400 truncate max-w-xl" title={log.message}>{log.message}</td>
                  </tr>
                ))
              )}
            </tbody>
