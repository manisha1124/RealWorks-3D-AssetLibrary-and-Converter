...
for i, line in enumerate(lines):
    if line.startswith("def heuristic_texture_linking(input_dir, dest_folder):"):
        new_lines.append("def heuristic_texture_linking(input_dir, dest_folder, asset_dir):\n")
        continue
...
CRITICAL: If the names are completely arbitrary and meaningless (e.g. 'Mat_001' and 'IMG_123.jpg') and you cannot semantically map them with absolute confidence, DO NOT GUESS. Instead, return exactly: {{"REQUIRE_VLM": true}}\"\"\"
        ...
        if llm_map and llm_map.get("REQUIRE_VLM"):
            ...
            color_legend = []
            orig_colors = {}
            for idx, mat in enumerate(mat_objects):
                orig_colors[mat] = tuple(mat.diffuse_color)
                r, g, b, a, name = palette[idx % len(palette)]
...
"""
    ...
    if skip:
        if "elif os.path.exists(texture_map_path):" in line:
            skip = False
